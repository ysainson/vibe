...
- **Dispatch**: write the review brief to a scratch file outside the repo (the session temp dir — never into the working tree), then run `node <companion> task --prompt-file <file> --effort xhigh` as a Claude background task (Bash `run_in_background`) — read-only (no `--write`), no `--model` flag: config owns the model; `task` is the invocation that accepts an explicit `--effort`.
